...
{ // Функция инхронизации времени с NTP сервером
...
label:
  ...
  if (DOW == "Mon")
    dayOfWeek = 1; // определяем номер дня недели
  if (DOW == "Tue")
    dayOfWeek = 2;
  if (DOW == "Wed")
    dayOfWeek = 3;
  ...
  if (MON == "Jan")
    month = 1; // определяем номер месяца
  if (MON == "Feb")
    month = 2;
  if (MON == "Mar")
    month = 3;
  ...
  Serial.println("ESP:\t" + String(hours) + ":" + String(minutes) + ":" + String(seconds));
